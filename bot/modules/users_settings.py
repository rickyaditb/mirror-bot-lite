...
async def get_user_settings(from_user, stype="main"):
    ...
    if stype == "leech":
        ...
        if (
            ...
            buttons.data_button(
                "Disable Hybrid Leech", f"userset {user_id} tog HYBRID_LEECH f"
            )
        elif TgClient.IS_PREMIUM_USER:
...
@new_task
async def add_file(client, message, ftype="THUMBNAIL"):
    user_id = message.from_user.id
    handler_dict[user_id] = False
    if ftype == "THUMBNAIL":
        fpath = f"thumbnails/{user_id}.jpg"
        await create_thumb(message, user_id)
    else:
        fpath = f"rclone/{user_id}.conf"
        await message.download(file_name=fpath)
    update_user_ldata(user_id, ftype, fpath)
    await delete_message(message)
    await database.update_user_doc(user_id, ftype, fpath)
...
async def add_one(_, message, option):
    ...
    user_dict.setdefault(option, {})
    try:
        val = eval(message.text)
        if isinstance(val, dict):
            user_dict[option].update(val)
            await delete_message(message)
...
async def get_menu(option, message, user_id):
    ...
    user_dict = user_data.get(user_id, {})
    buttons = ButtonMaker()
    if option in ["THUMBNAIL", "RCLONE_CONFIG"]:
        key = "file"
    else:
        key = "set"
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await query.answer()
    elif data[2] in ["leech", "rclone", "uploaders"]:
        await query.answer()
        ...
        if data[2] == "set":
            ...
            text = f"Add one or more string key and value to {data[3]}. Example: {{'key 1': 'val1', 'key 2': 'val2'}}. Timeout: 60 sec"
            func = add_one
        elif data[2] == "rmone":
        ...
        if data[3] in ["THUMBNAIL", "RCLONE_CONFIG"]:
            if data[3] == "THUMBNAIL":
                fpath = thumb_path
            else:
                fpath = rclone_conf

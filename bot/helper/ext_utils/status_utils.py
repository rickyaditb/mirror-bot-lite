...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    for index, task in enumerate(
        ...
        if (
            ...
            if (
                ...
                and getattr(task.listener, "is_torrent", False)
            ):
                try:
    ...
    free_space = disk_usage(DOWNLOAD_DIR).free if ospath.exists(DOWNLOAD_DIR) else disk_usage(".").free
    msg += f"<b>CPU:</b> {cpu_percent()}% | <b>FREE:</b> {get_readable_file_size(free_space)}"
